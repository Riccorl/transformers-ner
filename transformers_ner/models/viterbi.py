...
class ViterbiDecoder:
    def __init__(
        ...
        build_start_transition_matrix: bool = True,
    ):
        self.labels = labels
        ...
        self.start_transition_matrix = (
            ...
            if build_start_transition_matrix
            else None
        )
    ...
    def __call__(
        ...
        Args:
            tag_sequence (:obj:`torch.Tensor`):
                A tensor of shape (sequence_length, num_tags) representing scores for
                a set of tags over a given sequence.
            transition_matrix (:obj:`torch.Tensor`, optional, defaults to :obj:`None`):
                A tensor of shape (num_tags, num_tags) representing the binary potentials
                for transitioning between a given pair of tags.
            tag_observations (:obj:`List[int]`, optional, defaults to :obj:`None`):
                ...
                lies with the user.
            allowed_start_transitions (:obj:`torch.Tensor`, optional, defaults to :obj:`None`):
                An optional tensor of shape (num_tags,) describing which tags the START token
                may transition *to*. If provided, additional transition constraints will be used for
                determining the start element of the sequence.
            allowed_end_transitions (:obj:`torch.Tensor`, optional, defaults to :obj:`None`):
                An optional tensor of shape (num_tags,) describing which tags may transition *to* the
                end tag. If provided, additional transition constraints will be used for determining
                the end element of the sequence.
            top_k : (``int``, optional, defaults to :obj:`None`):
        ...
        if transition_matrix is None and self.tranistion_matrix is None:
            raise ValueError(
                "Viterbi decoding requires a transition matrix."
                "Use ``build_transition_matrix`` if you do not want to construct one."
            )
    ...
    def _make_transition_matrix(self, labels: Labels, namespace: str):
        """
        Build the transition matrix for the viterbi algorithm.

        Returns:
            :obj:`torch.Tensor`: The transition matrix.
        """
        transition_matrix = []
        for i in range(labels.get_label_size(namespace)):
            previous_label = labels.get_label_from_index(i, namespace)
            transitions = []
            for j in range(labels.get_label_size(namespace)):
                label = labels.get_label_from_index(j, namespace)
                if i != j and label[0] == "I" and not previous_label == "B" + label[1:]:
                    transitions.append(float("-inf"))
                else:
                    transitions.append(0.0)
            transition_matrix.append(transitions)
        return torch.as_tensor(transition_matrix)

    def _make_start_transition_matrix(self, labels: Labels, namespace: str):
        """
        Build the start transition matrix for the viterbi algorithm.

        Returns:
            :obj:`torch.Tensor`: The start transition matrix.
        """
        start_transitions = []
        for i in range(labels.get_label_size(namespace)):
            label = labels.get_label_from_index(i, namespace)
            if label[0] == "I":
                start_transitions.append(float("-inf"))
            else:
                start_transitions.append(0.0)
        return torch.as_tensor(start_transitions)
